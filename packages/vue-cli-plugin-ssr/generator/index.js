...
module.exports = (api, options, rootOptions) => {
	...
	api.extendPackage({
		scripts: {
			...
			'ssr:start':
				'cross-env NODE_ENV=production vue-cli-service ssr:serve --mode production'
		},
		dependencies: {
	...
	api.onCreateComplete(() => {
		// Main
		{
			...
			if (file) {
				...
				contents = contents.replace(
					/import router from ('|")\.\/router(\.\w+)?('|")/,
					`import { createRouter } from $1./router$3`
				);
				contents = contents.replace(
					/import store from ('|")\.\/store(\.\w+)?('|")/,
					`import { createStore } from $1./store$3`
				);
				contents = contents.replace(
				...
				contents = contents.replace(
					/const apolloProvider = createProvider\(({(.|\s)*?})?\)\n/,
					``
				);
				contents = contents.replace(
					/new Vue\({((.|\s)*)}\)\.\$mount\(.*?\)/,
		...
		if (api.hasPlugin('vuex')) {
          ...
          return new Vuex.Store({$1})
				...
				contents = contents.replace(
					/state:\s*{((.|\s)*?)},\s*(getters|mutations|actions|modules|namespaced):/,
					`state () {
          return {$1}
        },
        $3:`
		...
		api.exitLog(
			`Start dev server with ${chalk.cyan(
				`${hasYarn() ? 'yarn' : 'npm'} run ssr:serve`
			)}`,
			'info'
		);
		api.exitLog(
			`Build with ${chalk.cyan(
				`${hasYarn() ? 'yarn' : 'npm'} run ssr:build`
			)}`,
			'info'
		);
		api.exitLog(
			`Run in production with ${chalk.cyan(
				`${hasYarn() ? 'yarn' : 'npm'} run ssr:start`
			)}`,
			'info'
		);
	});
...
function getFile(api, file) {
	...
	api.exitLog(
		`File ${file} not found in the project. Automatic generation will be incomplete.`,
		'warn'
	);
}
