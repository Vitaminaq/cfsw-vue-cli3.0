...
export default defineComponent({
  ...
  created() {
    ...
    const { startIndex, cacheHeight, itemMap, scrollMap, virtualTop } =
  ...
  methods: {
    ...
    listen() {
      ...
      if (overIndex !== this.lastOver && overIndex) {
        // 计算滚动的条目
        const n = Math.ceil(
          Math.abs((overIndex - this.lastOver) / this.buffer)
        );
        ...
        if (isNext) {
          if (
            this.endIndex - overIndex <= this.buffer &&
            this.endIndex - overIndex > 0
          ) {
    ...
    getOver(sn: number) {
      ...
      while (i < this.endIndex) {
        const r = this.getItemMap(i);
        if (sn > scrollNum && sn < scrollNum + r) break;
        i++;
