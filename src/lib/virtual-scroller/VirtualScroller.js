...
export class VirtualScroller extends VirtualRepeater {
	constructor(config) {
		...
		this._childrenRO = new ResizeObserver((entries) =>
			this._childrenSizeChanged(entries)
		);
	...
	_updateView() {
		...
		if (this._scrollTarget === this._containerElement) {
			...
			const xMin = Math.max(
				...
				Math.min(
					scrollerWidth,
					containerBounds.left - scrollBounds.left
				)
			);
			const yMin = Math.max(
				0,
			...
			const xMax =
				this._layout.direction === 'vertical'
					? Math.max(
							...
							Math.min(
								scrollerWidth,
								containerBounds.right - scrollBounds.left
							)
					  )
					: scrollerWidth;
			const yMax =
