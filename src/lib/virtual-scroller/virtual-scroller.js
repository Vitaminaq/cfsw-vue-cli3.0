...
export class VirtualScrollerElement extends HTMLElement {
	constructor() {
		...
		this[_updateElement] = (element, item) =>
...
<slot></slot>`;
	...
	[_render]() {
		...
		const direction = layoutAttr.startsWith('horizontal')
			? 'horizontal'
			: 'vertical';
		const layout =
			scroller.layout instanceof Layout &&
			scroller.layout.direction === direction
				? scroller.layout
				: new Layout({ direction });

		let { createElement, updateElement, recycleElement } = this[
			_elementSource
		];
		if (!createElement) {
		...
		if (this.updateElement && !updateElement) {
			updateElement = this[_elementSource].updateElement = (
				element,
				index
			) =>
				this.updateElement(
					element,
					this.itemSource[_item](index),
					index
				);
		}
		if (this.recycleElement && !recycleElement) {
			recycleElement = this[_elementSource].recycleElement = (
				element,
				index
			) =>
				this.recycleElement(
