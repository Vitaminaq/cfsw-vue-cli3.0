export class VirtualRepeater {
	...
	set first(idx) {
		...
		const newFirst = Math.max(
			0,
			Math.min(idx, this._totalItems - this._num)
		);
	...
	_measureChildren({ indices, children }) {
		...
		const mm /** @type {{number: {width: number, height: number}}} */ = pm.reduce(
			(out, cur, i) => {
	...
	_render() {
		...
		if (rangeChanged || this._needsReset) {
			this._last =
				this._first +
				Math.min(this._num, this._totalItems - this._first) -
				1;
